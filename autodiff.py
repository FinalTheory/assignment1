...
class MulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        return [mul_op(output_grad, node.inputs[1]),
                mul_op(output_grad, node.inputs[0])]
...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        """
        return [matmul_op(output_grad, node.inputs[1], node.matmul_attr_trans_A, not node.matmul_attr_trans_B),
                matmul_op(node.inputs[0], output_grad, not node.matmul_attr_trans_A, node.matmul_attr_trans_B)]
...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        # Sum up all gradients passed from outputs of this node
        grad = sum_node_list(node_to_output_grads_list[node])
        # Which is exactly the gradient of output_node wrt this node
        node_to_output_grad[node] = grad
        # With this gradient, compute the gradients of output_node wrt
        # all inputs of this node, but notice that for each of these
        # input nodes, we only got one path from output_node to it.
        # Thus, we need the sum_node_list operation above to sum up
        # all those contributions from each path to output_node.
        grads = node.op.gradient(node, grad)
        if not grads:
            continue
        for n, g in zip(node.inputs, grads):
